...
with tab3:
    ...
    raw_text = st.text_area("Paste your messy block text here:", height=200, placeholder="e.g., US5446891A1992-02-261995-08-29International Business...")

    if st.button("🪄 Convert to Table"):
        if raw_text:
            # Clean out the header row if the user accidentally copied it
            clean_text = raw_text.replace("Publication numberPriority datePublication dateAssigneeTitle", "")

            # THE MAGIC REGEX:
            # 1. ([A-Z]{2}[A-Z0-9\*]*?) -> Finds 2 letters (country code), then any alphanumeric chars. 
            #    The '?' makes it stop IMMEDIATELY when it hits the next rule.
            # 2. (\d{4}-\d{2}-\d{2}) -> The strict YYYY-MM-DD date. This acts as our wall.
            pattern = r'([A-Z]{2}[A-Z0-9\*]*?)\s*(\d{4}-\d{2}-\d{2})\s*(\d{4}-\d{2}-\d{2})'
            matches = re.finditer(pattern, clean_text)

            extracted_data = []
            for match in matches:
                # Clean up the publication number (remove asterisks or accidental spaces)
